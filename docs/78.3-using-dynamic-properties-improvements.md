...
| 12 | `@DataJpaTest` instead of `@SpringBootTest` | Loads only the JPA slice, so it is faster. Needs `@AutoConfigureTestDatabase(replace = NONE)` so Boot does not swap in H2 | ✅ done |
...
- **`MySqlTest`** extends the base class:
  - **#12** - `@DataJpaTest` with `@AutoConfigureTestDatabase(replace = NONE)` (Boot 4 package: `org.springframework.boot.jdbc.test.autoconfigure`). The JPA slice only loads JPA components, so the test `@Import`s `BootstrapData`, autowires it and calls `run()` in `@BeforeEach` (safe to call repeatedly: it only inserts into empty tables). Flyway still runs in the slice.
