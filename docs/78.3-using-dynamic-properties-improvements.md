...
| 6 | `assertThat(beers).isNotEmpty()` | Reads more directly than `hasSizeGreaterThan(0)` | 💡 cosmetic |
...
| 8 | `@Testcontainers(disabledWithoutDocker = true)` | Skips the test instead of failing when Docker is missing - but see the caveat below | 💡 optional at most |
| 9 | Pin the image to MySQL 8.4 LTS, or to what production uses | `9.2` is on MySQL's short-lived "innovation" track, while the local server is `8.0.27`, so tests do not run against the version actually used | 💡 worth deciding |
| 10 | A dedicated test profile instead of `localmysql` | See the explanation below | 💡 recommended |
...
| 12 | `@DataJpaTest` instead of `@SpringBootTest` | Loads only the JPA slice, so it is faster. Needs `@AutoConfigureTestDatabase(replace = NONE)` so Boot does not swap in H2 | 💡 optional |
| 13 | Container reuse (`withReuse(true)`) or a shared singleton container | Each test class starts its own MySQL, which gets slow as container tests multiply | 💡 later, when there are more |

**Top 3 for now:** #10, #11, #9. #3 and #7 arrive with the next lectures anyway.
